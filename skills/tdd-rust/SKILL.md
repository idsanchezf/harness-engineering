...
Corre solo el test de la tarea actual, igual que en RED/GREEN — no el suite completo.
El suite completo (`cargo test`) corre UNA sola vez, al terminar todas las tareas de la
HU, no en cada ciclo individual.

### 4. Al completar todas las tareas de la HU

```bash
cargo test
# → VERDE: test result: ok (unica corrida del suite completo de la HU)
```
...
- Corre solo el test de la tarea actual despues de cada ciclo RED-GREEN-REFACTOR; el suite completo (`cargo test`) corre UNA sola vez, al terminar todas las tareas de la HU
