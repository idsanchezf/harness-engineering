...
function parseTrackArgs(argv) {
  ...
  const args = { subcommand: argv[0], hu: null, feature: null, out: null, force: false };
...
function runCollect(args, projectDir) {
  const result = collect({ projectDir, hu: args.hu, feature: args.feature, force: args.force });
...
function run(argv) {
  ...
  try {
    if (args.subcommand === 'collect') {
      ...
      console.error(`Subcomando desconocido: "${args.subcommand}". Uso: track collect [--hu F001:US-001 | --feature F001] [--force] | track dashboard [--out docs/tracking/dashboard.html]`);
      process.exitCode = 1;
    }
  } catch (err) {
